...
Push the current branch to the remote and create a GitHub Pull Request using the topic's spec and plan as context. This command is **publish-only** — it performs no local file changes. All local work (including the reference document) must be committed before running this command.

## Usage

```
/dev:pr    Push and open a PR for the current topic
```
...
| `{{reference_doc}}` | read from `topics[topic].refDoc` in dev-context.json (set by `/dev:docs` on save). If absent, fall back to `git log --diff-filter=AM -- docs/specs/ -1 --name-only` and validate the result matches `^docs/specs/[a-zA-Z0-9_-]+\.md$` before use. |
...
```
Then re-run `/dev:docs` and `/dev:pr` in sequence.
...
다음: PR이 병합되면 /dev:done 으로 아카이브하세요.
PR 수정이 필요하면 /dev:pr 을 다시 실행하세요 (docs:generated로 복귀 후 재push).
```

## Key Principles

- **Gate: docs:generated** — requires reference document to be committed before PR is created
- **Publish-only** — no local file changes; all commits must be done before this command
...
After `/dev:pr` completes (`pr:created`):

- **PR 제목·body만 수정**: `/dev:pr` 재실행 — `pr:created` 상태에서 자동으로 re-entry(update) 모드로 진입해 `gh pr edit`을 실행한다.
- **참조 문서도 수정 필요**: `pr:created → docs:generated`로 상태를 복귀시킨 뒤 `/dev:docs`와 `/dev:pr`을 순서대로 실행한다.
  ```bash
  node .harness/scripts/dev-context.js update-state \
    --topic=<topic> --phase=docs --status=generated
  ```
  그 다음 `/dev:docs` (참조 문서 재생성 + commit) → `/dev:pr` (재push + PR 업데이트).
- **병합 후**: `/dev:done` 으로 아카이브하고 토픽을 제거한다.
