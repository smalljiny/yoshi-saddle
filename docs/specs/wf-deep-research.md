...
`adapter-deep-research`는 ECC 원본 `deep-research` 스킬을 하네스 인프라 위에서 재구현한 워크플로우 스킬이다. 원본이 MCP 도구(`firecrawl_search`, `web_search_exa`)를 직접 호출하는 것과 달리, 이 스킬은 skill-registry에서 `[search-adapter]` 태그를 가진 어댑터 스킬을 동적으로 탐색하고, 각 어댑터의 Search Procedure 섹션에 위임하여 HTTP 로직 중복을 제거한다.
...
```
.claude/skills/adapter-deep-research/
...
Step 3에서 성공한 어댑터 중 우선순위 1위(`adapter-exa` > `adapter-firecrawl` > 기타)를 대표 어댑터로 선택한다. 대표 어댑터 실패 시 다음 건강한 어댑터로 폴백한다.

| 어댑터 | Content 오퍼레이션 | 호출 방식 |
|--------|-----------------|----------|
| adapter-firecrawl | `/v1/scrape` | 단일 URL — URL마다 반복 |
| adapter-exa | `/contents` | JSON 배열 — 전체 URL 일괄 호출 |
...
| `.claude/skills/adapter-exa/SKILL.md` | Exa REST API 어댑터 (`[search-adapter, exa]`) |
| `.claude/skills/adapter-firecrawl/SKILL.md` | Firecrawl REST API 어댑터 (`[search-adapter, firecrawl]`) |
| `docs/specs/meta/skill-registry.md` | skill-registry 참조 문서 |
| `docs/specs/search-adapters/exa.md` | adapter-exa 참조 문서 |
| `docs/specs/search-adapters/firecrawl.md` | adapter-firecrawl 참조 문서 |
