...
> `/dev:docs` 커맨드가 스펙-구현 정합성을 확인하고 기존 `docs/specs/` 파일을 업데이트(또는 신규 생성)한 뒤 단일 커밋으로 저장하는 계약.

## 개요

`/dev:docs`는 `/dev:review`와 `/dev:pr` 사이에 위치한 독립 단계이다. 이 커맨드가 `docs/specs/<name>.md` 참조 문서를 **최초 생성**하는 책임을 진다. `/dev:done`의 문서 생성 경로(Step 4.1–4.5)는 이 단계를 건너뛴 경우의 폴백이다. 다음 세 단계로 동작한다:
...
docs/
└── specs/
    └── <name>.md             # /dev:docs가 업데이트하거나 생성하는 참조 문서 (git-tracked)
...
`refDoc`은 항상 대표 문서 1개의 경로를 가진다. 업데이트된 파일이 여러 개이면 사용자가 선정한 대표 문서 1개를 기록한다. `/dev:pr`이 PR body의 `{{reference_doc}}`에 이 경로를 삽입한다.
...
2. **`/dev:verify` 통과 확인**: 세션 내 최근 통과 기록이 없으면 계속 여부(`y/n`)를 묻는다.
