...
> `/flow-plan`은 확정된 스펙을 기반으로 구현 계획(`implementation-plan.md`)을 생성하고, Codex plan-review 루프를 통해 계획을 확정한다. `config.plan.auto_review=true`이면 리뷰 루프가 자동으로 실행된다.

## 개요

플랜 작성은 세 단계로 구성된다. **planner 에이전트**가 스펙에서 구현 계획을 생성하고, **`/flow-plan` 커맨드**가 파일 이동·상태 전환·plan-review 루프를 제어한다. plan-review가 READY 또는 READY WITH NOTE를 반환하면 `/flow-impl`로 넘어간다.
...
- `topics[<topic>].plan`: Step 6에서 `/flow-plan`이 `set-field`로 기록
...
2.5. **의존성 분석 (JS/TS, best-effort)** — `package.json` 존재 시 `adapter-dependency-analysis` 스킬 실행
3. **active로 이동** — `backlog/<topic>/` → `active/<topic>/`, `dev-context.json` 경로 업데이트
4. **planner 에이전트 호출** — 스펙 + 의존성 분석 결과를 입력으로 `implementation-plan.md` 생성. planner는 `Write` 도구로 파일을 직접 저장하며, 메인 에이전트는 저장을 대행하지 않는다 (general-planning 모드 직접 호출은 Write 경로를 사용하지 않고 plan을 텍스트로 반환). planner는 자체 워크플로우 5단계(P1~P5)를 `TaskCreate`/`TaskUpdate`로 진행 표시한다 — 상세는 [planner-progress-tracking.md](planner-progress-tracking.md)
...
7. **Codex plan-review** — `plan:reviewing`으로 전환 후 `config.plan.auto_review` 확인:
   ...
   - **`true`**: `current_topic`을 `<topic>`으로 동기화 + plan 경로 검증 후 `adapter-codex-review` 스킬 자동 실행. 최대 3회 루프 — NOT READY면 Required Fixes 반영(TRUST BOUNDARY 준수) + `plan:ready` 전환 → 재시도; READY면 Step 8 `plan:confirmed` 분기로; READY WITH NOTE면 Notes 반영(사실 오류·누락 컨텍스트·구조적 결함만, 스타일 제외) 후 Step 8로. 3회 초과 또는 Availability Gate 실패·스킬 비정상 종료 시 수동 폴백.
8. **재진입 상태 테이블** — 상태에 따라 `/flow-impl` 안내 또는 재플랜 안내

### 재진입 상태 테이블

| `phase:status` | `planReview` Decision | Action |
|---|---|---|
| `plan:confirmed` | READY / READY WITH NOTE | `/flow-impl` 시작 가능 |
...
auto-review loop (최대 3회):
  ...
  3. adapter-codex-review 스킬 로드 → codex exec 실행
...
- **plan:confirmed는 Codex 소유** — `/flow-plan`은 `plan:confirmed`를 직접 설정하지 않는다; Codex `plan-review` 스킬이 소유한다
