# dev-context 전역 config + `/dev:impl` auto_start 플래그

> `dev-context.json`의 전역 `config` 섹션과 `dev-context.js` 점 경로 CLI, 그리고 `/dev:impl`이 소비하는 첫 config 키 `config.dev_impl.auto_start`의 동작 계약.

## 개요

`dev-context.json`은 토픽 라이프사이클 상태 외에 하네스 운영 설정을 담는 전역 `config` 섹션을 갖는다. `dev-context.js` CLI는 점 경로 `config.<namespace>.<key>`(깊이 2 고정)를 `read` · `set-field` 서브커맨드로 읽고 쓴다. `/dev:impl`은 `config.dev_impl.auto_start`를 조회하여 Pre-work Briefing 이후 승인 대기 여부를 결정한다. 값이 문자열 `"true"`이면 브리핑에 가시성 라인을 덧붙이고 승인 없이 구현 단계로 진입하며, 그 외 값에서는 기존 승인 대기 동작을 보존한다.
...
- 첫 번째 키: `config.dev_impl.auto_start`. 값은 boolean. 미설정일 때는 "기본 동작(승인 대기)"으로 해석된다.
...
### `/dev:impl` — `config.dev_impl.auto_start` 소비
