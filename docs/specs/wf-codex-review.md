...
`codex exec` 인자에 사용하는 경로는 반드시 사전에 검증한다. 빈 값, 절대 경로(`/`), 선행 대시, 디렉토리 순회(`../`), 제어 문자를 거부한 뒤 `greadlink -f` → `python3 os.path.realpath` 폴백으로 정규화하고 repo 루트 내부인지 확인한다.

**보안 노트**: 더블 쿼팅은 셸 재평가를 방지하지만 LLM 레벨 프롬프트 인젝션을 막지는 않는다. 경로가 repo 내부로 한정되고(`REPO_ROOT/*` 검사) `workspace-write` sandbox가 `codex exec` 동작을 제한함으로써 위험을 경계한다.
...
**plan-review** (경로 인자 불필요 — `current_topic`·`plan` 필드 자동 해석):
...
- **macOS 호환성**: path 정규화는 `greadlink -f` → `python3` 폴백. timeout은 `gtimeout` → `timeout` → no-op 순 자동 선택.
