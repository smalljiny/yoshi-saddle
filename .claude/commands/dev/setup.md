...
```
/dev:setup git    Detect git remotes and save config.git.* fields
```

Running `/dev:setup` without a subcommand, or with an unrecognized subcommand, shows:
```
지원되는 서브커맨드: git
사용법: /dev:setup git
```
...
⚠ upstream remote가 존재하지만 연결할 수 없습니다. Non-fork 패턴으로 fallback합니다.
  원인 확인 후 /dev:setup git 을 다시 실행하세요.
```
...
Use `^(feature|fix|chore)/` as the default — same value used by `/dev:pr` when `config.git.branchPattern` is not set.
...
If any `set-field` call fails, show the error and stop. The fields saved before the failure remain in dev-context.json; run `/dev:setup git` again to complete or correct all four fields.
...
이제 /dev:pr, /dev:docs, /dev:review가 이 값을 사용합니다.
```
...
- **No SessionStart automation** — `/dev:setup git` must be run explicitly; no hook auto-detection
